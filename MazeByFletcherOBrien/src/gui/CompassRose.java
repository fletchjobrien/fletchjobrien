...
public class CompassRose {
    ...
    private static final int CIRCLE_HIGHLIGHT = 935475; 
    //Color.decode("#115740").darker();// = new Color(1.0f, 1.0f, 1.0f, 0.8f); 
    private static final int CIRCLE_SHADE = 1468749; 
     ...
     */
    public void paintComponent(MazePanel mp) {
        ...
        */
         ...
         */
        drawBackground(mp);
        drawArms(mp, armLength, armWidth);
        drawBorderCircle(mp, width); // note: not currently visible due to color settings
        drawDirectionMarker(mp, width);
    }
     ...
     */
	private void drawArms(final MazePanel mp, final int length, final int width) {
        ...
        // draw each arm
        drawArmNorth(mp, length, width, x, y);
        drawArmEast(mp, length, width, x, y);
        drawArmSouth(mp, length, width, x, y);
        drawArmWest(mp, length, width, x, y);
	}
     ...
     * @param g2 The graphics object to draw on
     */
	private void drawBackground(final MazePanel mp) {
		// color setting hard coded as white
		mp.setColor(16777215); //color white
	 ...
	 */
	private void drawArmWest(MazePanel mp, int length, int width, int[] x, int[] y) {
	 ...
	 * @param y For the y coordinates of the triangle points, first entry given and fixed
	 */
	private void drawArmEast(MazePanel mp, int length, int width, int[] x, int[] y) {
	 ...
	 * @param y For the y coordinates of the triangle points, first entry given and fixed
	 */
	private void drawArmSouth(MazePanel mp, int length, int width, int[] x, int[] y) {
		// x[0] and y[0] are already set to the coordinates of the center point
		// set coordinates for 2nd and 3rd point and draw filled triangle
		x[1] = centerX;
        y[1] = centerY + length;
	 ...
	 * @param y For the y coordinates of the triangle points, first entry given and fixed
	 */
	private void drawArmNorth(MazePanel mp, int length, int width, int[] x, int[] y) {
	 ...
	 * @param width
	 */
	private void drawBorderCircle(MazePanel mp, int width) {
		...
		// draw both arcs
		mp.setColor(CIRCLE_SHADE);
        mp.addArc(x, y, w, w, 45, 180);
        mp.setColor(CIRCLE_HIGHLIGHT);
	 ...
	 */
	private void drawDirectionMarker(MazePanel mp, int width) {
		...
		drawMarker(mp, centerX, centerY - offset, "N");

		mp.setColor((CardinalDirection.East == currentDir) ? MARKER_COLOR : goldWM);
		drawMarker(mp, centerX + offset, centerY, "E");
		...
		drawMarker(mp, centerX - offset, centerY, "W");
	 ...
	 */
    private void drawMarker(MazePanel mp, float x, float y, String str) {
