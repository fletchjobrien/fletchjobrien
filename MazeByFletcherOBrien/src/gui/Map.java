...
public class Map {
	...
	private void drawMap(MazePanel mp, int px, int py, int walkStep, 
		...
		mp.setColor(255255255); //white is r = 255, g = 255, b = 255
		...
		for (int y = minY; y <= maxY; y++)
			for (int x = minX; x <= maxX; x++) {
				...
				if (x < mazeWidth)
					drawHorizontalLine(mp, showMaze, x, y, startX, startY);
				if (y < mazeHeight)
					drawVerticalLine(mp, showMaze, x, y, startX, startY);
			}
	 ...
	 */
	private void drawVerticalLine(MazePanel mp, boolean showMaze, int x, int y, int startX, int startY) {
		if (hasAVerticalWall(x, y) && 
				(seenWalls.hasWall(x, y, CardinalDirection.West) || showMaze)) {
			mp.setColor(seenWalls.hasWall(x, y, CardinalDirection.West) ? 255255255 : 160160160); //grey = 160160160
			mp.addLine(startX, startY, startX, startY - mapScale); 
		}
	}
	 ...
	 * @param startY the y coordinate for drawing
	 */
	private void drawHorizontalLine(MazePanel mp, boolean showMaze, int x, int y, int startX,
			int startY) {
		if (hasAHorizontalWall(x, y) && (seenWalls.hasWall(x,y, CardinalDirection.North) || showMaze) ) {
			mp.setColor(seenWalls.hasWall(x,y, CardinalDirection.North) ? 255255255 : 160160160);
			mp.addLine(startX, startY, startX + mapScale, startY); 
		}
	}
	...
	private void drawCurrentLocation(MazePanel mp, int viewDX, int viewDY) {
		...
		mp.addFilledOval(centerX-diameter/2, centerY-diameter/2, diameter, diameter);
	...
	private void drawArrow(MazePanel mp, int viewDX, int viewDY, 
		...
		mp.addLine(startX, startY, tipX, tipY);
		...
		*/
		...
		mp.addLine(tipX, tipY, tmpX + offsetX, tmpY + offsetY);
		mp.addLine(tipX, tipY, tmpX - offsetX, tmpY - offsetY);
	}
